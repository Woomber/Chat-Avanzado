...
public class Funcion_Principal extends JFrame_Principal {
    ...
    public Funcion_Principal() {
        ...
        receiver.onOneGroupUpdate = (int ID) -> UpdateOneGroup(ID);
        ...
        try {
            ...
            for (Integer i : ids) {
                ...
                if (stat) {
                    ...
                    JComponent_Grupo grupo = new JComponent_Grupo(nombreGrupo, Integer.valueOf(id));
    ...
    private void UpdateOneGroup(int ID) {
        PanelGrupos.revalidate();
        PanelGrupos.repaint();
        transmitter.setAction((Socket socket, PrintWriter pw, BufferedReader read) -> reloadGroup(socket, pw, read, ID));
        transmitter.StartThread();
    }

    private void reloadGroup(Socket socket, PrintWriter pw, BufferedReader read, int ID) {
        int index = 0;
        for (Component c : PanelGrupos.getComponents()) {
            JComponent_Grupo com = (JComponent_Grupo) c;
            if (com.getID() == ID) {
                try {
                    pw.println(JsonParser.paqueteToJson(new GrupoRequest(com.getID())));
                    Paquete grupoPaquete = JsonParser.jsonToPaquete(read.readLine());
                    String[] miembrosGrupo = JsonParser.jsonToStrings(grupoPaquete.getValue(GrupoResponse.PARAM_MIEMBROS));
                    JComponent_Grupo newGroup = new JComponent_Grupo(com.getGroupName(), com.getID());
                    newGroup.setOnInformationEnter(() -> {
                        this.setCursor(Cursor.HAND_CURSOR);
                    });
                    newGroup.setOnInformationLeave(() -> {
                        this.setCursor(Cursor.DEFAULT_CURSOR);
                    });
                    MensajeSerializable[] a = JsonParser.jsonToMensajes(grupoPaquete.getValue(GrupoResponse.PARAM_MENSAJES));
                    for (MensajeSerializable m : a) {
                        DocumentManager.SaveMessage(com.getID() + "_" + com.getGroupName(), m.getOrigen(), m.getMensaje(), true);
                    }
                    newGroup.setOnInformationClick(() -> LoadGroupConverataion(String.valueOf(com.getID()), com.getGroupName(), miembrosGrupo));
                    newGroup.setBtnEliminarClick(() -> iniciarBorrarGrupo(String.valueOf(com.getID())));
                    PanelGrupos.remove(com);
                    PanelGrupos.add(newGroup);
                    PanelGrupos.revalidate();
                    break;
                } catch (JsonParserException | IOException ex) {
